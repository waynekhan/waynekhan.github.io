...
However, on a work trip I realized that for whatever reason, bridged mode simply wouldn't work with my wireless network card, so I was forced to using a Linux Desktop Environment (LDE), something that I've long resisted. In doing so, I quickly realized that Alt-Tab no longer worked as my web browser was in the host OS (i.e., Windows), whilst Terminal was in the guest OS. All this to say that my productivity went down, as switching windows now required an additional mouse movement/click.
