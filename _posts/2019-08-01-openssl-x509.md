...
We'll be using `openssl` to generate a private key. This key manifests in the form of a file, so we'll call it a .key file:
...
Your CA will then use your CSR to generate a public certificate (e.g., a .pem file), typically Base 64-encoded, which you'll use in conjunction with `hostname.key`.

If you `cat` this file, the first and last lines will be  `-----BEGIN CERTIFICATE-----` and `-----END CERTIFICATE-----`.
